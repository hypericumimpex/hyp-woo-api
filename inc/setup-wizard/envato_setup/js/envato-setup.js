var EnvatoWizard = (function($){
    ...
    function window_loaded(){
        ...
        $('.sample-content').on('click', function() {
            if($(this).hasClass('pgs-woo-api-imported') ){
				return;
			}
		...
		*/
		...
		function import_demo_content( btn ) {
			...
			$.ajax({
				...
				data: {
					action:'pgs_woo_api_plugin_import_sample',
					sample_id: version,
...
jQuery(document).ready(function() {
    var nativeapp = envato_setup_params.ciyashop_native;
    if(nativeapp == 'android'){
        jQuery('.android-frm-field-group').show();
        jQuery('.ios-frm-field-group').hide();    
    } else {
        jQuery('.android-frm-field-group').hide();
        jQuery('.ios-frm-field-group').show();
    }
    jQuery('.ciyashop-native-app').on('click',function(){        
        var native_app = jQuery(this).val();
        if(native_app == 'android'){
            jQuery('.android-frm-field-group').show();
            jQuery('.ios-frm-field-group').hide();    
        } else {
            jQuery('.android-frm-field-group').hide();
